...
resource "google_iam_workload_identity_pool_provider" "gh_provider" {
  ...
  # Map the claims we’ll reference
  attribute_mapping = {
  ...
  # Lock to your GitHub org/user (adjust if needed)
  attribute_condition = "attribute.repository_owner=='alexanderlin-com'"

  oidc { issuer_uri = "https://token.actions.githubusercontent.com" }
}

# Allow your repo to impersonate the CI SA
...
output "workload_identity_provider" { value = google_iam_workload_identity_pool_provider.gh_provider.name }
