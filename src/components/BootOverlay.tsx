...
export default function BootOverlay({ onDone }: { onDone: () => void }) {
    ...
    (async () => {
      try {
        ...
        const lines = text
          ...
          .filter(Boolean)
          .map((l) =>
            l.length > BOOT.maxChars ? l.slice(0, BOOT.maxChars) + "…" : l
          );
  ...
  const STEP_MS = useMemo(
    () =>
      Math.max(
        25,
        Math.min(80, Math.floor(BOOT.durationMs / Math.max(1, display.length)))
      ),
    [display.length]
  );
  ...
  // Advance lines (no sound)
  useEffect(() => {
  if (disabled) return;
  const onPrefs = () => {
    if (typeof window !== "undefined" && localStorage.getItem(SKIP_KEY)) {
      finish();
    }
  };
...
useEffect(() => {
  const onStorage = (e: StorageEvent) => {
    if (e.key === SKIP_KEY && e.newValue) finish();
  };
  ...
  }, [disabled, visible, reduced, STEP_MS, display.length]);
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      className="fixed inset-0 z-50 bg-black text-cyan-200"
    >
      <div ref={containerRef} className="absolute inset-0 overflow-y-auto p-4">
        <pre className="font-mono text-sm leading-5 whitespace-pre-wrap">
          {
            display
              .slice(0, Math.max(1, Math.min(idx, display.length)))
              .join("\n")
          }
        </pre>
      </div>
